...
class ImprovedUNet3D(nn.Module):
    def __init__(self, in_channels=1, out_channels=3, base_filters=16, dropout=0.3):
        ...
        self.loc3 = LocalizationModule(base_filters*8 + base_filters*4, base_filters*4)
        self.loc2 = LocalizationModule(base_filters*4 + base_filters*2, base_filters*2)
        self.loc1 = LocalizationModule(base_filters*2 + base_filters, base_filters)
        
        # Deep supervision outputs
        self.ds3 = nn.Conv3d(base_filters*4, out_channels, kernel_size=1)
        self.ds2 = nn.Conv3d(base_filters*2, out_channels, kernel_size=1)
        self.ds1 = nn.Conv3d(base_filters, out_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        u3 = upsample_repeat(e4)
        u3 = torch.cat([u3, e3], dim=1)
        l3 = self.loc3(u3)
        
        u2 = upsample_repeat(l3)
        u2 = torch.cat([u2, e2], dim=1)
        l2 = self.loc2(u2)
        
        u1 = upsample_repeat(l2)
        u1 = torch.cat([u1, e1], dim=1)
        l1 = self.loc1(u1)
        
        # Deep supervision: sum outputs from different levels
        out = F.interpolate(self.ds3(l3), size=l1.shape[2:], mode='trilinear', align_corners=False)
        out += F.interpolate(self.ds2(l2), size=l1.shape[2:], mode='trilinear', align_corners=False)
        out += self.ds1(l1)
